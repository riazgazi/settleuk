...
const PrepTab = () => {
    ...
    const CategoryTabs = () => (
        <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 14, paddingBottom: 2 }}>
            {QA_CATEGORIES.map(c => (
                <button key={c.id} onClick={() => { setQaCategory(c.id); setOpenQ(null); }} style={{ flex: "none", padding: "7px 12px", borderRadius: 20, cursor: "pointer", whiteSpace: "nowrap", border: `1px solid ${qaCategory === c.id ? "#4A90D9" : "rgba(255,255,255,0.1)"}`, background: qaCategory === c.id ? "rgba(74,144,217,0.18)" : "rgba(255,255,255,0.03)", color: qaCategory === c.id ? "#4A90D9" : "rgba(255,255,255,0.45)", fontSize: 11.5, fontWeight: qaCategory === c.id ? 700 : 500 }}>
                    {c.icon} {c.label}
                </button>
            ))}
        </div>
    ...
    const QACard = ({ item, i, accentColor }) => {
        ...
        return (
            <div style={{ marginBottom: 8, borderRadius: 12, overflow: "hidden", border: `1px solid ${isOpen ? accentColor + "55" : "rgba(255,255,255,0.07)"}` }}>
                <div onClick={() => setOpenQ(isOpen ? null : i)} style={{ padding: "13px 14px", cursor: "pointer", background: isOpen ? accentColor + "0e" : "rgba(255,255,255,0.03)", display: "flex", alignItems: "flex-start", gap: 10 }}>
                    <div style={{ width: 22, height: 22, borderRadius: "50%", background: accentColor + "22", border: `1.5px solid ${accentColor}55`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, color: accentColor, flexShrink: 0, marginTop: 1 }}>{i + 1}</div>
            ...
            {section === "cas" && (
                <div>
                    <div style={{ background: "rgba(91,141,239,0.08)", border: "1px solid rgba(91,141,239,0.25)", borderRadius: 14, padding: "14px 16px", marginBottom: 16 }}>
                        ...
                        <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.55)", lineHeight: 1.65 }}>CAS (Confirmation of Acceptance for Studies) interview সাধারণত হয় না — এটা একটা document/number। তবে কিছু ক্ষেত্রে university বা UKVI আপনাকে <strong style={{ color: "#4A90D9" }}>Genuine Student Interview</strong>-এর জন্য ডাকতে পারে। এই interview-এ তারা verify করে যে আপনি সত্যিই পড়তে আসছেন।</div>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 12 }}>
                            {[["🎯 কে নেয়?", "UKVI Entry Clearance Officer"], ["📍 কোথায়?", "VFS/UKVCAS Centre, Dhaka"], ["⏱️ কতক্ষণ?", "১০-২০ মিনিট"], ["📅 কখন?", "Visa application-এর পর"]].map(([label, val], i) => (
                                <div key={i} style={{ background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "8px 10px" }}><div style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", marginBottom: 3 }}>{label}</div><div style={{ fontSize: 12, fontWeight: 700, color: "#F0F4FF" }}>{val}</div></div>
                            ))}
                        </div>
                    </div>
                    <div style={{ background: "rgba(232,168,56,0.07)", border: "1px solid rgba(232,168,56,0.2)", borderRadius: 12, padding: "11px 14px", marginBottom: 14, fontSize: 12, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>⚠️ <strong style={{ color: "#E8A838" }}>Interview Tips:</strong> সব documents সাথে রাখুন। সত্য কথা বলুন। Clear ও confident হন। Nervous হলেও pause নিয়ে ভেবে উত্তর দিন।</div>
                    <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, color: "rgba(255,255,255,0.28)", marginBottom: 10 }}>Possible Questions — tap to see answers</div>
                    <CategoryTabs />
                    {filterByCategory(CAS_QA).map((item, i) => <QACard key={i} item={item} i={i} accentColor={C.blue} />)}
                    {filterByCategory(CAS_QA).length === 0 && (<div style={{ textAlign: "center", padding: "20px 10px", color: "rgba(255,255,255,0.25)", fontSize: 12.5 }}>No questions in this category yet.</div>)}
                </div>
            )}
            {section === "ukvi" && (
                <div>
                    <div style={{ background: "rgba(61,184,139,0.08)", border: "1px solid rgba(61,184,139,0.25)", borderRadius: 14, padding: "14px 16px", marginBottom: 16 }}>
                        ...
                        <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.55)", lineHeight: 1.65 }}>UK Visas and Immigration (UKVI) কিছু applicant-কে <strong style={{ color: "#1D9E6A" }}>Credibility Interview</strong>-এর জন্য ডাকে। এটা mandatory নয় — শুধু যাদের application-এ কোনো concern দেখা যায় তাদের ডাকা হয়। Interview-এ তারা জানতে চায় আপনি সত্যিকারের student কিনা।</div>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 12 }}>
                    ...
                    <div style={{ background: "rgba(232,91,91,0.07)", border: "1px solid rgba(232,91,91,0.2)", borderRadius: 12, padding: "11px 14px", marginBottom: 14, fontSize: 12, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>🔴 <strong style={{ color: "#CF142B" }}>সতর্কতা:</strong> UKVI interview-এ মিথ্যা বললে permanent ban হতে পারে। সব সময় সত্য কথা বলুন, এমনকি যদি তা আপনার বিরুদ্ধেও যায়।</div>
                    <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, color: "rgba(255,255,255,0.28)", marginBottom: 10 }}>Common Questions — tap to see answers</div>
                    <CategoryTabs />
                    {filterByCategory(UKVI_QA).map((item, i) => <QACard key={i} item={item} i={i} accentColor={C.green} />)}
                    {filterByCategory(UKVI_QA).length === 0 && (<div style={{ textAlign: "center", padding: "20px 10px", color: "rgba(255,255,255,0.25)", fontSize: 12.5 }}>No questions in this category yet.</div>)}
                </div>
            )}
            {section === "refusal" && (
                <div>
                    <div style={{ background: "rgba(232,91,91,0.07)", border: "1px solid rgba(232,91,91,0.25)", borderRadius: 14, padding: "14px 16px", marginBottom: 16 }}>
                        ...
                        <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.55)", lineHeight: 1.65 }}>Refusal letter-এ একটি <strong style={{ color: "#CF142B" }}>refusal code</strong> থাকে যা কারণ নির্দেশ করে। এই code বুঝে পরবর্তী application শক্তিশালী করুন। নিচে সবচেয়ে সাধারণ refusal codes এবং সমাধান দেওয়া হলো।</div>
                    </div>
                    ...
                    <div style={{ marginTop: 20 }}>
                        ...
                        {pdfFiles.length === 0 ? (
                            <div style={{ textAlign: "center", padding: "20px 16px", color: "rgba(255,255,255,0.25)", fontSize: 12.5 }}>কোনো PDF upload হয়নি। আপনার refusal letter বা সংশ্লিষ্ট document এখানে upload করুন।</div>
                        ) : (
                            pdfFiles.map(f => (
                                <div key={f.id}>
                        ...
                        <div style={{ marginTop: 8, fontSize: 11, color: "rgba(255,255,255,0.22)", textAlign: "center", lineHeight: 1.5 }}>🔒 সব PDF শুধু আপনার device-এ সংরক্ষিত — কোনো server-ে upload হয় না</div>
                    </div>
                </div>
            )}
        </div>
