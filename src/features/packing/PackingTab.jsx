    ...
    return (
        <div>
            <div style={{ background: "rgba(91,141,239,0.08)", border: "1px solid rgba(91,141,239,0.25)", borderRadius: 14, padding: "14px 16px", marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 12.5, color: "rgba(255,255,255,0.55)", lineHeight: 1.6 }}>Plan and track everything you need to pack for the UK. Check off items as you pack them, and monitor your luggage weight against the airline limit.</div>
            </div>
                ...
                return (
                    <div key={cat.id} style={{ marginBottom: 10, borderRadius: 14, overflow: "hidden", border: `1px solid ${isOpen ? cat.color + "55" : "rgba(255,255,255,0.08)"}` }}>
                        <div onClick={() => setOpenCat(isOpen ? null : cat.id)} style={{ padding: "13px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, background: isOpen ? cat.color + "0e" : "rgba(255,255,255,0.03)" }}>
                            ...
                            <div style={{ flex: 1 }}><div style={{ fontSize: 13.5, fontWeight: 700, color: "#F0F4FF" }}>{cat.name}</div><div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)" }}>{catChecked} / {cat.items.length} packed</div></div>
                                ...
                                {cat.items.map(item => {
                                    ...
                                    return (
                                        <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                                            ...
                                            <div onClick={() => toggleItem(item.id)} style={{ flex: 1, cursor: "pointer" }}><span style={{ fontSize: 13, color: checked[item.id] ? "rgba(255,255,255,0.35)" : "#F0F4FF", textDecoration: checked[item.id] ? "line-through" : "none" }}>{item.name}</span></div>
