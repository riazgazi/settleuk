...
import { UNI_TUITION_MAP, COST_FIXED, EXPENSE_CAT_META } from "../../data/costs";
...
const CostsTab = () => {
    ...
    const byMonth = {};
    [...filteredExpenses].reverse().forEach(exp => { const m = exp.date.slice(3); if (!byMonth[m]) byMonth[m] = []; byMonth[m].push(exp); });
    ...
    const addExpense = () => {
        ...
        setExpenses(p => [...p, { id: Date.now(), name: expName.trim(), amt, cat: expCat, phase: expPhase, date: new Date().toLocaleDateString("en-GB") }]);
    ...
    return (
        <div>
            <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
                <div onClick={() => setPanel("calc")} style={{ flex: 1, padding: "10px 8px", background: panel === "calc" ? C.greenDim : C.surface2, border: `1px solid ${panel === "calc" ? C.green : C.border}`, borderRadius: 10, cursor: "pointer", fontSize: 12, fontWeight: 700, textAlign: "center", color: panel === "calc" ? C.green : C.textMuted }}>🧮 Calculate Your Cost</div>
                <div onClick={() => setPanel("track")} style={{ flex: 1, padding: "10px 8px", background: panel === "track" ? C.greenDim : C.surface2, border: `1px solid ${panel === "track" ? C.green : C.border}`, borderRadius: 10, cursor: "pointer", fontSize: 12, fontWeight: 700, textAlign: "center", color: panel === "track" ? C.green : C.textMuted }}>📊 Expense Tracker</div>
            </div>
            {panel === "calc" && (
                <div>
                    <div style={card}>
                ...
                <div>
                    <div style={{ display: "flex", background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 10, padding: 3, gap: 3, marginBottom: 14 }}>
                        {[["all", "🌐 All Journey", C.green], ["bd", "🇧🇩 Pre-Arrival", C.amber], ["uk", "🇬🇧 In UK", C.blue]].map(([id, lbl, color]) => (
                            <button key={id} onClick={() => setPhase(id)} style={{ flex: 1, padding: "8px 4px", border: "none", borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: "pointer", background: phase === id ? C.surface : "none", color: phase === id ? color : C.textMuted, boxShadow: phase === id ? "0 1px 4px rgba(0,0,0,.4)" : "none" }}>{lbl}</button>
                        ))}
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: 14 }}>
                        <div style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 8px" }}><div style={{ fontSize: 9, color: C.textMuted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 3 }}>Total Spent</div><div style={{ fontSize: 15, fontWeight: 800, color: C.green }}>{fmtGBP(totalSpent)}</div><div style={{ fontSize: 10, color: C.textMuted, marginTop: 1 }}>{gbp2bdt(totalSpent, rate)}</div></div>
                        <div style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 8px" }}><div style={{ fontSize: 9, color: C.textMuted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 3 }}>🇧🇩 Pre-Arrival</div><div style={{ fontSize: 15, fontWeight: 800, color: C.amber }}>{fmtGBP(preSpent)}</div><div style={{ fontSize: 10, color: C.textMuted, marginTop: 1 }}>{gbp2bdt(preSpent, rate)}</div></div>
                        <div style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 8px" }}><div style={{ fontSize: 9, color: C.textMuted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 3 }}>🇬🇧 In UK</div><div style={{ fontSize: 15, fontWeight: 800, color: C.blue }}>{fmtGBP(ukSpent)}</div><div style={{ fontSize: 10, color: C.textMuted, marginTop: 1 }}>{gbp2bdt(ukSpent, rate)}</div></div>
                    </div>
                    <div style={{ ...card, padding: "12px 14px" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5, fontSize: 11, color: C.textMuted }}><span>Spent: {fmtGBP(totalSpent)}</span><span>Budget: {fmtGBP(grandTotal)}</span></div>
                        <div style={{ height: 6, background: C.surface2, borderRadius: 3, overflow: "hidden" }}><div style={{ height: "100%", width: `${budgetPct}%`, background: barColor, borderRadius: 3, transition: "width 0.5s ease" }} /></div>
                    ...
                    <div style={{ ...card, padding: "12px 14px" }}>
                        ...
                        {Object.keys(catTotals).length === 0 ? (<div style={{ fontSize: 12, color: C.textDim, textAlign: "center", padding: "8px 0" }}>No expenses yet</div>) : (
                            Object.entries(catTotals).sort((a, b) => b[1] - a[1]).map(([c, amt]) => {
                                ...
                                return (
                                    <div key={c} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                                        ...
                                        <div style={{ flex: 1, height: 5, background: C.surface2, borderRadius: 3, overflow: "hidden" }}><div style={{ height: "100%", width: `${pct}%`, background: meta.color, borderRadius: 3, transition: "width 0.5s ease" }} /></div>
                    ...
                    <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 12, paddingBottom: 2 }}>
                        {[["all", "All"], ["visa", "🛂 Visa"], ["ielts", "📝 IELTS"], ["travel", "✈️ Travel"], ["tuition", "🎓 Tuition"], ["living", "🏠 Living"], ["food", "🍜 Food"], ["mobile", "📱 Mobile"], ["transport", "🚌 Transport"], ["health", "💊 Health"], ["other", "📦 Other"]].map(([id, lbl]) => (
                            <div key={id} onClick={() => setCat(id)} style={{ flex: "0 0 auto", padding: "5px 12px", background: cat === id ? C.greenDim : C.surface2, border: `1px solid ${cat === id ? C.green : C.border}`, borderRadius: 20, cursor: "pointer", fontSize: 11, fontWeight: 600, color: cat === id ? C.green : C.textMuted, whiteSpace: "nowrap" }}>{lbl}</div>
                        ))}
                    </div>
                    <div style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 12, padding: 14, marginBottom: 14 }}>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                            <select value={expCat} onChange={e => setExpCat(e.target.value)} style={fInput}>
                                <option value="visa">🛂 Visa</option><option value="ielts">📝 IELTS</option><option value="travel">✈️ Travel</option><option value="tuition">🎓 Tuition</option><option value="living">🏠 Living</option><option value="food">🍜 Food</option><option value="mobile">📱 Mobile</option><option value="transport">🚌 Transport</option><option value="health">💊 Health</option><option value="other">📦 Other</option>
                            </select>
                            <select value={expPhase} onChange={e => setExpPhase(e.target.value)} style={fInput}>
                                <option value="bd">🇧🇩 Pre-Arrival (BD)</option><option value="uk">🇬🇧 In UK</option>
                            </select>
                                            ...
                                            <div key={exp.id} style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 12, padding: "11px 12px", display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                                                ...
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    ...
                                                    <div style={{ fontSize: 10, color: C.textMuted, marginTop: 2, display: "flex", alignItems: "center", gap: 5 }}>
                                                        <span style={{ display: "inline-block", fontSize: 9, fontWeight: 700, padding: "1px 5px", borderRadius: 4, background: exp.phase === "uk" ? "rgba(74,158,255,.15)" : "rgba(245,158,11,.15)", color: exp.phase === "uk" ? C.blue : C.amber }}>{exp.phase === "uk" ? "🇬🇧 In UK" : "🇧🇩 Pre-Arrival"}</span>
                                                        {exp.cat} · {exp.date}
                                                    </div>
                                                </div>
