...
import { UNI_BANDS_BACHELOR, UNI_BANDS_MASTERS, SEARCH_LINKS, getUniBand } from "../../data/universities";

const UniversityFinder = ({ onClose, onSaveProfile, savedAcademic }) => {
    const [step, setStep] = useState("intro");
    const [level, setLevel] = useState(savedAcademic?.level || "");
    const [form, setForm] = useState(savedAcademic || { ssc: "", hsc: "", bachelorSubject: "", cgpa: "", ielts: "", course: "", budget: "" });

    const inputStyle = { width: "100%", padding: "11px 14px", marginBottom: 14, background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, color: "#fff", fontSize: 14, outline: "none", fontFamily: "inherit", boxSizing: "border-box" };
    ...
    const btn = (bg, disabled) => ({ width: "100%", padding: "13px 0", background: disabled ? "#2a2a2a" : bg, border: "none", borderRadius: 12, color: disabled ? "rgba(255,255,255,0.3)" : "#fff", fontSize: 14.5, fontWeight: 800, cursor: disabled ? "default" : "pointer" });

    const gpaOk = level === "bachelor" ? form.ssc && form.hsc : level === "masters" ? form.bachelorSubject && form.cgpa : false;
    const relevantGpa = level === "masters" ? parseFloat(form.cgpa) : (parseFloat(form.ssc) + parseFloat(form.hsc)) / 2;
    const band = (level && gpaOk) ? getUniBand(level, relevantGpa, UNI_BANDS_BACHELOR, UNI_BANDS_MASTERS) : null;
            ...
            <div style={{ width: "100%", maxWidth: 460, background: "#0A2545", borderRadius: "24px 24px 0 0", padding: "24px 22px 28px", border: "1px solid rgba(255,255,255,0.1)", maxHeight: "88vh", overflowY: "auto" }}>
                ...
                {step === "intro" && (
                    <div>
                        ...
                        <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 13, lineHeight: 1.6, marginBottom: 22, textAlign: "center" }}>Complete your academic profile to receive university and course suggestions based on your grades.</p>
                ...
                {step === "form" && (
                    <div>
                        ...
                        <h2 style={{ color: "#fff", fontSize: 18, fontWeight: 800, marginBottom: 4 }}>{level === "masters" ? "📘 Master's Profile" : "🎓 Bachelor's Profile"}</h2>
                ...
                {step === "results" && band && (
                    <div>
                        ...
                        <div style={{ background: "rgba(232,168,56,0.07)", border: "1px solid rgba(232,168,56,0.2)", borderRadius: 12, padding: "10px 14px", marginBottom: 18, fontSize: 11.5, color: "rgba(255,255,255,0.55)", lineHeight: 1.5 }}>💡 This is a rough estimate, not an admission guarantee. Always check each university's official entry requirements.</div>
