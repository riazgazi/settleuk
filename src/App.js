...
const STAGES = [
  ...
  },
  {
    ...
    insights: [
      ...
      { type: "info", title: "Priority service",          sub: "Priority visa service gives a decision in 5 working days if you're short on time." },
    ],
  },
  {
...
function ReadinessRing({ score, labelColor }) {
  ...
  return (
    <svg width={cx*2} height={cy*2} viewBox={`0 0 ${cx*2} ${cy*2}`} style={{ flexShrink: 0 }}>
      ...
      <circle cx={cx} cy={cy} r={r} fill="none" stroke={labelColor} strokeWidth="7"
        strokeDasharray={`${filled} ${circ}`} strokeLinecap="round"
        transform={`rotate(-90 ${cx} ${cy})`}
        style={{ transition: "stroke-dasharray 0.8s ease" }} />
      <text x={cx} y={cy-3}  textAnchor="middle" fill="#EEF2F7"              fontSize="15" fontWeight="800">{score}</text>
      <text x={cx} y={cy+11} textAnchor="middle" fill="rgba(255,255,255,0.3)" fontSize="8">/100</text>
    </svg>
  );
}
...
function ReadinessScore({ stIdx, taskDone, docChecked, sg }) {
  ...
  const score = Math.round((completedTasks / totalTasks) * 50)
  ...
  const labelColor = score >= 80 ? "#3DB88B"         : score >= 60 ? "#5B8DEF"   : score >= 40 ? "#E8A838"          : "#6B7A99";
  ...
  if (nextTarget) {
    ...
    if (completedDocs  < 4) hints.push("Mark key documents as ready");
    if (stIdx < 2)          hints.push("Progress your application stage");
  }

  return (
    <div style={{ marginBottom: 16, padding: "14px 16px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, display: "flex", gap: 16, alignItems: "center" }}>
      <ReadinessRing score={score} labelColor={labelColor} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 10.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, color: "rgba(255,255,255,0.3)", marginBottom: 3 }}>Readiness Score</div>
        <div style={{ fontSize: 13.5, fontWeight: 800, color: labelColor, marginBottom: 6 }}>{score} of 100 points · {label}</div>
        <div style={{ height: 5, background: "rgba(255,255,255,0.07)", borderRadius: 5, overflow: "hidden", marginBottom: 7 }}>
          <div style={{ height: "100%", width: `${score}%`, borderRadius: 5, transition: "width 0.6s ease",
            background: score >= 80 ? "linear-gradient(90deg,#3DB88B,#5BE8AC)" : score >= 60 ? "linear-gradient(90deg,#5B8DEF,#3DB88B)" : score >= 40 ? "linear-gradient(90deg,#E8A838,#5B8DEF)" : "linear-gradient(90deg,#534AB7,#E8A838)" }} />
        </div>
        ...
        {nextTarget && hints.length > 0 && (
          <div style={{ padding: "7px 10px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 9 }}>
            <div style={{ fontSize: 9.5, fontWeight: 700, color: "rgba(255,255,255,0.25)", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4 }}>To reach {nextTarget}/100</div>
            {hints.slice(0, 2).map((h, i) => (
              <div key={i} style={{ fontSize: 10.5, color: "rgba(255,255,255,0.4)", display: "flex", gap: 5, marginBottom: i < hints.length - 1 ? 3 : 0 }}>
...
function StatusDropdown({ statusId, onChange }) {
  ...
  return (
    <div ref={ref} style={{ marginBottom: 16, position: "relative" }}>
      <div style={{ fontSize: 10.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.6, color: "rgba(255,255,255,0.3)", marginBottom: 6 }}>
        Current Status · <span style={{ color: "rgba(255,255,255,0.2)", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>tap to change</span>
      </div>
      <button onClick={() => setOpen(o => !o)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", background: "rgba(61,184,139,0.1)", border: "2px solid rgba(61,184,139,0.55)", borderRadius: 13, cursor: "pointer", textAlign: "left", color: "#EEF2F7", boxShadow: "0 0 0 4px rgba(61,184,139,0.07)" }}>
        <style>{`@keyframes pulse{0%,100%{transform:scale(1);opacity:0.9}50%{transform:scale(2.4);opacity:0}}`}</style>
        <span style={{ position: "relative", width: 10, height: 10, flexShrink: 0 }}>
          <span style={{ position: "absolute", inset: 0, borderRadius: "50%", background: "#3DB88B", animation: "pulse 2s infinite" }} />
          <span style={{ position: "absolute", inset: 0, borderRadius: "50%", background: "#3DB88B" }} />
        </span>
        <span style={{ flex: 1, fontSize: 14, fontWeight: 800, color: "#3DB88B" }}>{current.emoji} {current.label}</span>
        <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", transition: "transform 0.2s", transform: open ? "rotate(180deg)" : "rotate(0deg)", display: "inline-block" }}>▼</span>
      </button>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, zIndex: 200, background: "#0D1E2F", border: "1px solid rgba(255,255,255,0.13)", borderRadius: 14, overflow: "hidden", boxShadow: "0 20px 50px rgba(0,0,0,0.6)" }}>
          {STATUSES.map(s => (
            <button key={s.id} onClick={() => { onChange(s.id); setOpen(false); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", background: s.id === statusId ? "rgba(61,184,139,0.1)" : "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,0.05)", color: s.id === statusId ? "#3DB88B" : "rgba(255,255,255,0.65)", fontSize: 13, fontWeight: s.id === statusId ? 800 : 500, cursor: "pointer", textAlign: "left" }}>
...
function CountdownCard({ arrivalDays }) {
  ...
  const color   = arrived ? "#3DB88B" : arrivalDays < 30 ? "#E85B5B" : arrivalDays < 90 ? "#E8A838" : "#3DB88B";
  const msg     = arrived ? "Welcome! Complete your settlement tasks."
    : arrivalDays < 30  ? "⚠️ Less than a month — stay on top of tasks!"
    : arrivalDays < 90  ? "Keep up momentum — you're getting close."
    : "Stay consistent and plan ahead.";
  return (
    <div style={{ marginBottom: 16, background: `linear-gradient(135deg,${color}0D 0%,rgba(24,95,165,0.06) 100%)`, border: `1.5px solid ${color}40`, borderRadius: 16, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14 }}>
      <div style={{ width: 58, height: 58, borderRadius: 14, flexShrink: 0, background: `${color}15`, border: `2px solid ${color}55`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        {arrived ? <span style={{ fontSize: 26 }}>🇬🇧</span> : <>
          <span style={{ fontSize: 10, fontWeight: 700, color, textTransform: "uppercase", letterSpacing: 0.4 }}>Days</span>
          <span style={{ fontSize: 24, fontWeight: 900, color, lineHeight: 1.1 }}>{arrivalDays}</span>
        </>}
      </div>
      <div>
        <div style={{ fontSize: 14, fontWeight: 800, color: "#EEF2F7", marginBottom: 3 }}>{arrived ? "You're in the UK! 🎉" : `${arrivalDays} Days Until UK Arrival`}</div>
        <div style={{ fontSize: 11.5, color: "rgba(255,255,255,0.4)", lineHeight: 1.5 }}>{msg}</div>
      </div>
    </div>
  );
}
    ...
    <div style={{ marginBottom: 16, padding: "14px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.35)", fontWeight: 600 }}>Step {stIdx + 1} of {total}</div>
        </div>
        ...
        {ALL_STAGE_NAMES.map((name, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5, zIndex: 1 }}>
              <div style={{ width: cur ? 26 : 18, height: cur ? 26 : 18, borderRadius: "50%", border: `2.5px solid ${done ? "#3DB88B" : cur ? sg.accentBtn : "rgba(255,255,255,0.15)"}`, background: done ? "#3DB88B" : cur ? sg.accentBtn + "33" : "rgba(255,255,255,0.04)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: cur ? 11 : 9, color: done ? "#fff" : cur ? sg.accentBtn : "rgba(255,255,255,0.25)", fontWeight: 800, boxShadow: cur ? `0 0 0 4px ${sg.accentBtn}22` : "none", transition: "all 0.3s" }}>
                {done ? "✓" : i + 1}
              </div>
              <span style={{ fontSize: 8.5, fontWeight: cur ? 800 : 500, color: cur ? sg.accentBtn : done ? "rgba(255,255,255,0.4)" : "rgba(255,255,255,0.18)", textAlign: "center", whiteSpace: "nowrap" }}>{name}</span>
            </div>
          );
        })}
      </div>
...
function NextBestAction({ sg, onStart }) {
  const uc = { critical: { bg: "rgba(232,91,91,0.08)", border: "rgba(232,91,91,0.3)", badge: "#E85B5B", badgeBg: "rgba(232,91,91,0.15)", label: "🔴 Urgent" }, high: { bg: "rgba(232,168,56,0.08)", border: "rgba(232,168,56,0.3)", badge: "#E8A838", badgeBg: "rgba(232,168,56,0.15)", label: "🟡 This Week" }, medium: { bg: "rgba(91,141,239,0.06)", border: "rgba(91,141,239,0.2)", badge: "#5B8DEF", badgeBg: "rgba(91,141,239,0.15)", label: "🔵 Upcoming" } }[sg.deadlineUrgency] || {};
  return (
    <div style={{ background: uc.bg, border: `1px solid ${uc.border}`, borderRadius: 16, padding: "14px 16px", marginBottom: 16 }}>
      ...
      <div style={{ fontSize: 11.5, color: "rgba(255,255,255,0.45)", marginBottom: 12, display: "flex", alignItems: "center", gap: 5 }}><span>🕒</span><span>{sg.deadline}</span></div>
      <button onClick={onStart} style={{ width: "100%", padding: "9px 0", background: sg.accentBtn, border: "none", borderRadius: 10, color: "#fff", fontSize: 13.5, fontWeight: 800, cursor: "pointer", boxShadow: `0 3px 12px ${sg.accentBtn}44` }}>Start Now →</button>
    </div>
  );
}
...
function QuickCard({ icon, title, sub, pct, onClick }) {
  return (
    <div onClick={onClick} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: "12px", cursor: "pointer" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: pct !== undefined ? 8 : 0 }}>
...
export default function App() {
  ...
  if (advancePrompt) {
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.78)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "Arial, sans-serif" }}>
        <div style={{ width: "100%", maxWidth: 380, background: "#0D1E2F", borderRadius: 22, padding: "28px 24px", border: "1px solid rgba(255,255,255,0.12)" }}>
          ...
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", lineHeight: 1.6, marginBottom: 20, textAlign: "center" }}>
            You completed:<br /><strong style={{ color: "#EEF2F7" }}>"{advancePrompt.taskText.slice(0, 55)}"</strong><br /><br />
            Update your status to:<br /><span style={{ color: "#3DB88B", fontWeight: 800, fontSize: 15 }}>{next.emoji} {next.label}</span>?
          </div>
          <button onClick={confirmAdvance} style={{ ...btnStyle("#3DB88B"), marginBottom: 10 }}>Yes, update my status ✓</button>
          <button onClick={() => setAdvancePrompt(null)} style={{ ...btnStyle("transparent"), border: "1px solid rgba(255,255,255,0.15)", color: "rgba(255,255,255,0.6)" }}>No, I'll update manually</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#08111C", fontFamily: "Arial, sans-serif", color: "#EEF2F7", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ width: "100%", maxWidth: 420, background: "rgba(255,255,255,0.05)", borderRadius: 24, padding: "32px 28px", border: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          <button disabled={!editProfile.name.trim() || !hasChanges} onClick={() => { if (editProfile.name.trim()) { setProfile(p => ({ ...p, ...editProfile })); setShowToast("✓ Changes Saved"); setTimeout(() => { setShowToast(""); setShowSettings(false); }, 1100); } }} style={btnStyle(editProfile.name.trim() && hasChanges ? "#3DB88B" : "#333")}>Save Changes ✓</button>
          ...
          <button onClick={() => { if (window.confirm("Are you sure? This will permanently delete all your progress.")) { localStorage.clear(); window.location.reload(); } }} style={{ width: "100%", padding: "12px 0", background: "transparent", border: "1px solid rgba(232,91,91,0.35)", borderRadius: 12, color: "#E85B5B", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Reset All Progress</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(160deg,#0B1E35 0%,#0D2A1F 50%,#1A0D2E 100%)", fontFamily: "Arial, sans-serif", padding: 20 }}>
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          {step === 1 && (<div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 24, padding: "32px 28px", border: "1px solid rgba(255,255,255,0.1)" }}>
          ...
          {step === 2 && (<div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 24, padding: "32px 28px", border: "1px solid rgba(255,255,255,0.1)" }}>
            ...
            {STATUSES.map(s => (<button key={s.id} onClick={() => setProfile(p => ({ ...p, statusId: s.id, step: 3 }))} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", marginBottom: 6, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, color: "#fff", fontSize: 13, cursor: "pointer", textAlign: "left" }}>
          ...
          {step === 3 && (<div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 24, padding: "32px 28px", border: "1px solid rgba(255,255,255,0.1)" }}>
  ...
  const insightMeta = (type) => ({
    ...
    bg:     type === "warn" ? "rgba(232,91,91,0.07)"   : type === "tip" ? "rgba(61,184,139,0.07)"  : "rgba(91,141,239,0.07)",
    border: type === "warn" ? "rgba(232,91,91,0.28)"   : type === "tip" ? "rgba(61,184,139,0.22)"  : "rgba(91,141,239,0.22)",
    color:  type === "warn" ? "#E85B5B"                : type === "tip" ? "#3DB88B"                 : "#5B8DEF",
    badge:  type === "warn" ? { label: "🔴 Urgent",   c: "#E85B5B", bg: "rgba(232,91,91,0.15)" }
          : type === "tip"  ? { label: "🟢 Tip",      c: "#3DB88B", bg: "rgba(61,184,139,0.13)" }
          :                   { label: "🟡 Important", c: "#E8A838", bg: "rgba(232,168,56,0.13)" },
  });
      ...
      <div style={{ background: "linear-gradient(135deg,#0B1E35 0%,#0D2A1F 100%)", borderBottom: "1px solid rgba(255,255,255,0.07)", padding: "14px 20px 0" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
            <div style={{ width: 38, height: 38, borderRadius: 10, flexShrink: 0, background: "linear-gradient(135deg,#1a3a5c,#0d4a2f)", border: "1.5px solid rgba(61,184,139,0.35)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 900, color: "#EEF2F7", letterSpacing: 0.5 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                ...
                <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>Hello, {profile.name}!</span>
              </div>
              {arrivalDays !== null && (
                <div style={{ fontSize: 11, fontWeight: 700, marginTop: 1, color: arrivalDays <= 0 ? "#3DB88B" : arrivalDays < 30 ? "#E85B5B" : arrivalDays < 90 ? "#E8A838" : "#3DB88B" }}>
                  {arrivalDays > 0 ? `${arrivalDays} days to arrival` : "You're in the UK now! 🇬🇧"}
                </div>
              )}
            </div>
            <button onClick={() => { setEditProfile({ name: profile.name, arrival: profile.arrival }); setShowSettings(true); }} style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "rgba(255,255,255,0.5)", cursor: "pointer", fontSize: 15, width: 34, height: 34, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>⚙️</button>
          </div>
          <div style={{ marginBottom: 10 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 12px", background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 20, fontSize: 11.5, fontWeight: 700, color: "#EEF2F7" }}>📍 {sg.name}</span>
          </div>
          <div style={{ display: "flex", overflowX: "auto" }}>
            {[["home","🏠","Home"],["tasks","✅","Tasks"],["docs","📄","Documents"],["guides","📖","Guides"]].map(([id,em,lbl]) => (
              <button key={id} onClick={() => setTab(id)} style={{ flex: "none", padding: "8px 14px", background: "transparent", border: "none", borderBottom: tab === id ? "2px solid #3DB88B" : "2px solid transparent", color: tab === id ? "#3DB88B" : "rgba(255,255,255,0.4)", fontSize: 12.5, fontWeight: tab === id ? 700 : 400, cursor: "pointer", whiteSpace: "nowrap", display: "flex", alignItems: "center", gap: 5 }}>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "16px 20px" }}>
        ...
        {tab === "home" && (
          <div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 10 }}>Welcome back, {profile.name} 👋</div>
            ...
            <div style={{ background: "rgba(61,184,139,0.07)", border: "1px solid rgba(61,184,139,0.17)", borderRadius: 12, padding: "10px 14px", marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div key={i} style={{ display: "flex", gap: 10, background: m.bg, border: `1px solid ${m.border}`, borderRadius: 12, padding: "10px 12px", marginBottom: 7 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                      ...
                      <span style={{ fontSize: 9, fontWeight: 700, color: m.badge.c, background: m.badge.bg, padding: "2px 7px", borderRadius: 20, letterSpacing: 0.3 }}>{m.badge.label}</span>
                    </div>
          ...
          <div>
            ...
            {sg.tasks.map(task => (
              <div key={task.id} onClick={() => toggleTask(task.id)} style={{ display: "flex", gap: 12, padding: "13px 14px", marginBottom: 7, background: taskDone[task.id] ? "rgba(61,184,139,0.06)" : "rgba(255,255,255,0.03)", border: `1px solid ${taskDone[task.id] ? "#3DB88B44" : "rgba(255,255,255,0.06)"}`, borderRadius: 12, cursor: "pointer", alignItems: "flex-start" }}>
                <div style={{ width: 20, height: 20, borderRadius: 6, flexShrink: 0, marginTop: 2, border: `2px solid ${taskDone[task.id] ? "#3DB88B" : "rgba(255,255,255,0.2)"}`, background: taskDone[task.id] ? "#3DB88B" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12 }}>{taskDone[task.id] && "✓"}</div>
                <div style={{ flex: 1 }}>
            ...
            <div style={{ marginTop: 16, padding: "11px 14px", background: "rgba(61,184,139,0.05)", border: "1px solid rgba(61,184,139,0.15)", borderRadius: 12, fontSize: 11.5, color: "rgba(255,255,255,0.4)", textAlign: "center" }}>
        ...
        {tab === "docs" && (
          <div>
            ...
            <p style={{ margin: "0 0 10px", fontSize: 12.5, color: "rgba(255,255,255,0.4)" }}>{docsReadyCount} of {DOCS.length} documents ready</p>
          ...
          <div>
            ...
            {GUIDES.map((g, i) => (
              <a key={i} href={g.url} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 14px", marginBottom: 7, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, textDecoration: "none", color: "#EEF2F7" }}>
                ...
                <div style={{ flex: 1 }}><div style={{ fontSize: 13.5, fontWeight: 700 }}>{g.title}</div><div style={{ fontSize: 10.5, color: "rgba(255,255,255,0.3)", marginTop: 2 }}>{g.sub}</div></div>
